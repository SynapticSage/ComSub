...
def bootstrap_means(df_clean: pd.DataFrame, base_query_str: str, additional_queries: list, 
                    statistic: Callable = np.mean, n_samples: int = 100, 
...
# ----------------------------------
def plot_bootstrap_grid(df_clean: pd.DataFrame, base_query_str: str):
    ...
    for i, row_condition in enumerate(unique_conditions):
        ...
        for j, col_condition in enumerate(unique_conditions):
            ...
            if i != j:
                ...
                plot_bootstrap_means(subset_df1.append(subset_df2), plot_ci=True, cmap='coolwarm', reverse_cmap=True, plot_zero_lines=True, ax=ax)
            ...
            # For diagonal plots, show the overall KDE and scatter plot for the condition using a constant black colormap
                ...
                plot_bootstrap_means(subset_df, plot_ci=False, cmap=ListedColormap(['black']), plot_zero_lines=True, ax=ax)
...
base_query_str =  'pattern_cca1 == 2 & pattern_cca2==10 & uv_components == 1'
plot_bootstrap_grid(df_clean,  base_query_str + " & pattern_class_name == 'theta'")
plt.supertitle("Bootstrap Resampled Means with Confidence Intervals - theta")
plot_bootstrap_grid(df_clean,  base_query_str + " & pattern_class_name == 'delta'")
plt.suptitle("Bootstrap Resampled Means with Confidence Intervals - delta")
plot_bootstrap_grid(df_clean,  base_query_str + " & pattern_class_name == 'ripple'")
plt.suptitle("Bootstrap Resampled Means with Confidence Intervals - ripple")
