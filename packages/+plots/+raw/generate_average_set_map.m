function generate_average_set_heatmap(selected_rows, selected, sets_wanna_plot, varargin)
%GENERATE_AVERAGE_SET_HEATMAP Generate average heatmap for a set of trials
    ...
    parse(p, selected_rows, sets_wanna_plot, varargin{:});
    
    split_by_reward = p.Results.split_by_reward;
    grid_res = p.Results.grid_res;

    % Grid discretization using full data
    ...
    % Setup the figure using the fig() function
    f = fig('Trajectory Heatmaps');

    for set_idx = 1:numSets
        % Extract the current set_to_plot parameters as you did before
        set_to_plot = sets_wanna_plot{set_idx};
        set_fields = fieldnames(set_to_plot);
        set_values = struct2cell(set_to_plot);
        title_str = strcat(set_fields, ':', cellstr(num2str(cell2mat(set_values))), ' ');

        inds_set = true(height(selected_rows), 1);
        for j = 1:length(set_fields)
            inds_set = inds_set & selected_rows.(set_fields{j}) == set_values{j};
        end
